...
# Source: https://github.com/vietnh1009/Tetris-deep-Q-learning-pytorch/tree/master
# Disclaimer: Rewards and state space modified from original
...
class Tetris:
    ...
    def get_state_properties(self, board):
        ...
        #row_transitions, col_transitions = self.get_row_and_column_transitions(board)
        #well_depth = self.get_cumulative_well_depth(board)
        #hole_depth = self.get_hole_depth(board)
        #return torch.FloatTensor([lines_cleared, holes, bumpiness, height, well_depth, row_transitions, col_transitions, hole_depth])
    
    def get_cumulative_well_depth(self, board): # ADDED
        board = np.array(board)
        num_rows, num_cols = board.shape
        cumulative_well_depth = 0
        ...
        for col in range(num_cols):
            ...
            for row in range(num_rows):
                if board[row, col] == 0:
                    ...
                    right_filled = (col == num_cols - 1) or (board[row, col + 1] == 1)
                    no_blocks_above = np.all(board[:row, col] == 0)

                    if no_blocks_above and (left_filled and right_filled):
                        well_depth += 1 
                else:
                    cumulative_well_depth += well_depth
                    well_depth = 0

            cumulative_well_depth += well_depth

        return cumulative_well_depth
    
    def get_row_and_column_transitions(self, board): # ADDED
        board = np.array(board)
        padded_board = np.pad(board, ((0, 0), (1, 1)), mode='constant', constant_values=0)        
        row_transitions = 0
        for row in padded_board:            
            row_transitions += np.sum(row[:-1] != row[1:])
        padded_board = np.pad(board, ((1, 1), (0, 0)), mode='constant', constant_values=0)
        column_transitions = 0
        for col in padded_board.T:
            column_transitions += np.sum(col[:-1] != col[1:])

        return row_transitions, column_transitions
    ...
    def get_hole_depth(self, board):
        ...
        for col in range(num_cols):
            filled_cells_above = 0 
            for row in range(num_rows):
                if board[row, col] == 0:
                    hole_depth += filled_cells_above
                else:
                    filled_cells_above += 1
    ...
    def step(self, action, render=True, video=None):
        ...
        score = 1 + (lines_cleared) * self.width # MODIFIED
        self.score += score
        self.tetrominoes += 1
